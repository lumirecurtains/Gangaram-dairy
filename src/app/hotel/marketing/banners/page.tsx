...
export default function BannersPage() {
  ...
  const loadBanners = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/v1/hotel/marketing/banners?merchantId=${merchantId}`, {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/v1/hotel/marketing/banners", {
        method: "POST",
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      const res = await fetch("/api/v1/hotel/marketing/banners", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ action: "delete", merchantId, bannerId: id }),
      });
  ...
  const toggleStatus = async (id: string, currentStatus: boolean) => {
    try {
      const token = await user?.getIdToken();
      await fetch("/api/v1/hotel/marketing/banners", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ action: "toggle_status", merchantId, bannerId: id, banner: { isActive: !currentStatus } }),
      });
      loadBanners();
    } catch (err: any) {
