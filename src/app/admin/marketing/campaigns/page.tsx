...
export default function CampaignsPage() {
  ...
  const loadCampaigns = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch("/api/v1/admin/marketing/global/campaigns", {
  ...
  const toggleStatus = async (id: string, currentStatus: boolean) => {
    try {
      ...
      const res = await fetch("/api/v1/admin/marketing/global/campaigns", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ action: "toggle_status", campaignId: id, campaign: { isActive: !currentStatus } }),
      });
      if (!res.ok) throw new Error((await res.json()).error);
      loadCampaigns();
    } catch (err: any) {
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: "var(--text-secondary)" }}>
            Schedule and manage campaigns that run across the entire platform.
          </p>
        </div>
        <button
